...
RSpec.feature "Creating a Business Finance Support Scheme", type: :feature do
  let(:document) { FactoryBot.create(:business_finance_support_scheme) }
  let(:content_id) { document["content_id"] }
  let(:document_type) { :business_finance_support_scheme }
  ...
  scenario "saving a new document with no data" do
    ...
    # Custom fields
    expect(page).to have_css(".govuk-error-message", text: "Continuation link can't be blank")
  ...
  scenario "saving a new document with valid data" do
    ...
    # Custom fields
    fill_in "#{document_type}[continuation_link]", with: "https://www.gov.uk"
    fill_in "#{document_type}[will_continue_on]", with: "Website name"
