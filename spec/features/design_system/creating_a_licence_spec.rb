...
RSpec.feature "Creating a Licence", type: :feature do
  let(:document) { FactoryBot.create(:licence_transaction) }
  let(:content_id) { document["content_id"] }
  let(:document_type) { :licence_transaction }
  ...
  before do
    log_in_as_design_system_editor(:licence_transaction_editor, organisation_content_id_for_authorized_user)
  ...
  scenario "saving a new document with valid data" do
    ...
    schema.facets.each do |facet|
      ...
      next if %w[licence_transaction_will_continue_on licence_transaction_continuation_link licence_transaction_licence_identifier].include? key
    ...
    # Custom fields for licence transactions (there is url validation on the link, and additional custom validation)
    fill_in "Name of website where users apply", with: "Website on GOV.UK"
