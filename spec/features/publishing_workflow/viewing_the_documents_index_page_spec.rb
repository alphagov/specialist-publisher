...
RSpec.feature "Viewing the documents index page", type: :feature do
  ...
  let(:cma_cases) do
    example_cases = 3.times.collect do |n|
      FactoryBot.create(
        :cma_case,
        ...
        "public_updated_at" => (test_date - (3 - n).days).iso8601,
      )
    end
    example_cases[1]["publication_state"] = "published"
    example_cases[1]["state_history"] = { "1" => "published" }
    example_cases[2]["publication_state"] = "draft"
    example_cases[2]["state_history"] = { "1" => "published", "2" => "unpublished", "3" => "draft" }
    example_cases
  end
