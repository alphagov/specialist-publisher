describe('LengthCounter', function () {
  var $fieldHtml, $warningHtml, field, longText
  ...
  beforeEach(function () {
    ...
    longText = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse faucibus sit' +
    ...
    shortText = longText.slice(0, 140)
  })
  ...
  describe('when the threshold is passed', function () {
    ...
    it('should display the amount of characters', function () {
      expect(field.$count.text()).toBe(`Current length: ${longText.length}`)
    })
  })
  ...
  describe('when characters are deleted after the threshold has been passed', function () {
    beforeEach(function () {
      field.$input.val(longText)
      field.$input.keyup()
      field.$input.val(shortText)
      field.$input.keyup()
    })

    it('should still display the warning with an updated count', function () {
      expect($('.summary-length-info:visible').length).toBe(1)
      expect(field.$count.text()).toBe(`Current length: ${shortText.length}`)
    })
  })
})
