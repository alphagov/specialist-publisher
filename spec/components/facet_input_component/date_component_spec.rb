...
RSpec.describe FacetInputComponent::DateComponent, type: :component do
  ...
  it "should render date field for partially filled date from document" do
    content_item = FactoryBot.create(:cma_case, :draft, title: "Example CMA Case")
    document = DocumentBuilder.build(CmaCase, content_item)
    document.opened_date = "2025"
    params = {
      "cma_case" => {
        "opened_date(1i)" => "2025",
        "opened_date(2i)" => "",
  ...
  it "should render date field for invalid partially filled date from document" do
    content_item = FactoryBot.create(:cma_case, :draft, title: "Example CMA Case")
    document = DocumentBuilder.build(CmaCase, content_item)
    document.opened_date = "not"
    params = {
      "cma_case" => {
        "opened_date(1i)" => "not",
        "opened_date(2i)" => "",
  ...
  it "should render date field for invalid filled date in params" do
    content_item = FactoryBot.create(:cma_case, :draft, title: "Example CMA Case")
    document = DocumentBuilder.build(CmaCase, content_item)
    document.opened_date = "not-a-date"
    params = {
      "cma_case" => {
        "opened_date(1i)" => "not",
        "opened_date(2i)" => "a",
