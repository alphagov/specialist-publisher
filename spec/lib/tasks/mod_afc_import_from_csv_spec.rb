...
RSpec.describe "mod_afc_import_from_csv", type: :task do
  ...
  it "imports documents from a CSV file" do
    ...
    expect { task.execute(csv_file_path: csv_path) }.to output(/Imported: 1 document\(s\).*Skipped: 0 row\(s\)/m).to_stdout
  end
  ...
  it "imports documents in reverse order" do
    ...
    ).and_return(afc_business_double).ordered
    expect(ArmedForcesCovenantBusiness).to receive(:new).with(
      "title": "ABC Company",
      "summary": generate_summary,
    ...
    expect { task.execute(csv_file_path: csv_path) }.to output(/Imported: 2 document\(s\).*Skipped: 0 row\(s\)/m).to_stdout
  end
  ...
  it "logs the details for invalid rows in dry run mode" do
    ...
    allow(CSV).to receive(:foreach).with(csv_path, headers: true).and_return(csv_data.each)
    afc_business_double = double(save: true)
